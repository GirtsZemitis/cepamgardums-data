...
It bakes the current data (sales chart, today, hourly, live stock) into one .html
that opens in any browser — no Python, no install. Interactive (tab switch, toggles,
day-picker) but FROZEN at export time: there is no live Refresh. Re-run to refresh.
...
Chart library loads from a CDN, so the recipient needs internet to render charts.
"""
import argparse
import datetime
import json
...
import app  # reuse the dashboard's data assembly (importing does NOT start the server)
...
<style>
  ...
  .status{font-size:12px;color:#9aa0a6;margin-left:6px}.note{color:#6b7280;font-size:12px}
...
<script>
const DATA=/*DATA*/null, HOURLY=/*HOURLY*/null, STOCK=/*STOCK*/null;
const COLORS=["#3b82f6","#ef4444","#10b981","#f59e0b","#a855f7"];
const $=id=>document.getElementById(id);
let metric="rev", gran="week", chart=null, hmetric="cnt", hourChart=null;
...
function weekly(){const labels=[],bucket=[];let cur=null;
  DATA.dates.forEach(d=>{const dt=new Date(d+"T00:00:00");const wd=(dt.getDay()+6)%7;
    ...
    if(k!==cur){cur=k;labels.push(k);}bucket.push(labels.length-1);});return{labels,bucket};}
function view(){if(gran==="day")return{labels:DATA.dates,series:DATA.series.map(s=>s[metric].slice())};
  const{labels,bucket}=weekly();return{labels,series:DATA.series.map(s=>{const v=Array(labels.length).fill(0);
...
function sets(){const vw=view();
  ...
  m.unshift({label:"Kopā",data:tot,borderColor:"#e8eaed",borderDash:[6,4],borderWidth:3,tension:.3,
    pointRadius:gran==="week"?3:0,pointHoverRadius:5,fill:false});return{labels:vw.labels,datasets:m};}
function draw(){const d=sets();if(chart){chart.data.labels=d.labels;chart.data.datasets=d.datasets;chart.update();return;}
  chart=new Chart($("chart"),{type:"line",data:d,options:{responsive:true,maintainAspectRatio:false,
    interaction:{mode:"index",intersect:false},plugins:{legend:{labels:{color:"#e8eaed",usePointStyle:true}},
...
function renderToday(){const t=DATA.today;$("todayhead").textContent="Pārdošana "+(t.date||"");
  const el=$("today");el.innerHTML="";
  if(!t.machines.length){el.innerHTML="<p class='note'>Šajā dienā nav pārdošanas.</p>";return;}
  t.machines.forEach((m,i)=>{const rows=m.products.map(p=>`<div class="row"><span>${p.name}</span><span class="q">${p.qty} × · €${p.rev.toFixed(2)}</span></div>`).join("");
    el.insertAdjacentHTML("beforeend",`<div class="mcard"><h3 style="color:${COLORS[i%5]}">${m.label}</h3><div class="tot">${m.qty} gab. · €${m.rev.toFixed(2)}</div>${rows}</div>`);});}
...
function drawHourly(){const d=$("hourdate").value;
  ...
  hourChart=new Chart($("hourchart"),{type:"bar",data:cfg,options:{responsive:true,maintainAspectRatio:false,
...
function renderStock(){const el=$("stock");
  ...
  el.innerHTML="";
  STOCK.machines.forEach((m,i)=>{const rows=m.products.map(p=>{const pct=p.capacity?p.remaining/p.capacity:1;
    ...
    el.insertAdjacentHTML("beforeend",`<div class="mcard"><h3 style="color:${COLORS[i%5]}">${m.label}</h3>
